...
let plugin = function (v) {
    ...
    function createElement(newNode, isSVG) {
        ...
        newNode.dom = isSVG ?
            document.createElementNS(svgstr, newNode.name) :
    ...
    function patch($parent, newTree, oldTree, isSVG, max, newNode, oldNode, i, isNew, name) {
        ...
        for (i = 0; i < max; i++) {
            ...
            if (newNode.nt === und || newNode.nt === 3) { // Is text
                if (newNode.nt === und) {
                    newTree[i] = newNode = {
                        name: '#text',
                        isVnode: true,
    ...
    v.update = function (args) {
        ...
        if (typeof args[0] === 'object') {
            if (args[0].isComponent === und && typeof args[0].view === 'function') {
