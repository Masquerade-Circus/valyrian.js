...
function addAttributes(newVnode: VnodeWithDom): void {
  for (let prop in newVnode.props) {
    if (reservedProps[prop]) {
      if (directives[prop] && directives[prop](newVnode.props[prop], newVnode) === false) {
        break;
      }
      continue;
    }
...
function patchNew(newParentVnode: VnodeWithDom): void {
  ...
  for (let i = 0; i < newTree.length; i++) {
    ...
    if (childVnode instanceof Vnode) {
      ...
      addAttributes(childVnode as VnodeWithDom);
      childVnode.props.oncreate && childVnode.props.oncreate(childVnode);
      patchNew(childVnode as VnodeWithDom);
...
function patch(newParentVnode: VnodeWithDom, oldParentVnode: VnodeWithDom): void {
  ...
  if (!oldTreeLength) {
    return patchNew(newParentVnode);
  }

  if (oldTreeLength && newTree[0] instanceof Vnode && oldTree[0] instanceof Vnode && "key" in newTree[0].props && "key" in oldTree[0].props) {
