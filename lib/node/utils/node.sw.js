...
const config = {
  ...
  // Critical resources that block installation
  criticalUrls: ["/"],
  // Optional resources cached in background (non-blocking)
  optionalUrls: []
};
...
// Background caching for optional resources (non-blocking)
async function cacheOptionalResources() {
  try {
    const cache = await caches.open(cacheName);
    for (const url of config.optionalUrls) {
      try {
        await cache.add(url);
      } catch (error) {
        Log("WORKER: optional cache failed for", url, error.message);
      }
    }
    Log("WORKER: optional resources cached");
  } catch (error) {
    Log("WORKER: optional caching failed", error);
  }
}
...
self.addEventListener("install", (event) => {
  ...
  event.waitUntil(
    caches
      .open(cacheName)
      .then((cache) => {
        return cache.addAll(config.criticalUrls);
      })
      .then(() => {
        Log("WORKER: critical resources cached, install completed.");
        // Cache optional resources in background (non-blocking)
        cacheOptionalResources();
      ...
      .catch((error) => {
        Log("WORKER: install failed", error);
      })
  );
});
