...
v.dom2vnode = dom => {
  if (dom.nodeType === 3) {
    return new TextVnode(dom.nodeValue, dom);
  }
...
function lifecycleCallRemove(vnode) {
  for (let i = 0, l = vnode.children.length; i < l; i++) {
    vnode.children[i] instanceof Vnode && lifecycleCallRemove(vnode.children[i]);
  }

  if (vnode.props[onremove]) {
    return vnode.props[onremove](vnode);
  }
}
...
function removeVnode(vnode) {
  if (vnode && vnode.dom) {
    vnode instanceof Vnode && lifecycleCallRemove(vnode);
...
function patch(parentNode, oldParentNode) {
  ...
  if (newTree.length === 0) {
    ...
    while (i--) {
      oldTree[i] instanceof Vnode && lifecycleCallRemove(oldTree[i]);
    }
    parentNode.dom.textContent = '';

    // Is keyed list
  } else if (oldTree.length && newTree[0].props.key) {
      ...
      if (newNode instanceof Vnode) {
        if (!oldNode) {
          ...
          if (newNode.name === oldNode.name) {
            ...
            oldNode instanceof Vnode && lifecycleCallRemove(oldNode);
            newNode.dom = createElement(newNode.name, newNode.isSVG);
            updateProps(newNode, emptyNode);
