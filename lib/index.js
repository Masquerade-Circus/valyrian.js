...
v.useState = function (initial) {
  if (currentParentNode.components === UND) {
    currentParentNode.components = [];
  }

  if (currentParentNode.components.indexOf(currentNode) === -1) {
    currentParentNode.components.push(currentNode);
  }

  let hook;
  let oldComponentNode = currentOldParentNode.components && currentOldParentNode.components[currentParentNode.components.length - 1];
  let oldMethod = oldComponentNode && ('view' in oldComponentNode.name ? oldComponentNode.name.view : oldComponentNode.name);

  if (currentNode.hooks === UND) {
    currentNode.hooks = [];
    hookIndex = 0;
  }

  if (oldMethod === currentMethod && 'hooks' in oldComponentNode && oldComponentNode.hooks[hookIndex] !== UND) {
    currentNode.hooks = oldComponentNode.hooks;
...
v.onCleanup = function (callback) {
  if (currentParentNode.onCleanup === UND) {
    currentParentNode.onCleanup = [];
  }
  currentParentNode.onCleanup.push(callback);
};

function cleanupVnode(vnode) {
  if ('onCleanup' in vnode) {
...
function patch(parentNode, oldParentNode, isSVG) {
  ...
  if (newTree.length === 0) {
    ...
    for (let i = 0; i < newTree.length; i++) {
      currentNode = newTree[i];

      if (currentNode instanceof Vnode) {
        if (typeof currentNode.name !== 'string') {
          currentMethod = 'view' in currentNode.name ? currentNode.name.view : currentNode.name;
          newTree.splice(i, 1, ...[currentMethod.call(currentMethod, currentNode.props, ...currentNode.children)]);
          i--;
        } else {
          if (currentNode.name === 'svg') {
