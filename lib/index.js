...
v.useHooks = () => useHooks = TRUE;

v.useState = function (initial) {
  let hook;
  let oldComponentNode = currentOldParentNode.components && currentOldParentNode.components[currentComponentIndex];
    ...
    for (let i = 0; i < newTree.length; i++) {
      ...
      if (currentNode instanceof Vnode) {
        if (typeof currentNode.name !== 'string') {
          // -> onComponent
          if (useHooks) {
            parentNode.hasComponents = TRUE;
            hookIndex = 0;
            currentComponentNode = currentNode;
            parentNode.components.push(currentNode);
            currentNode.isComponent = TRUE;
            currentNode.hooks = [];
          }

          let currentMethod = currentNode.name.view || currentNode.name;
