...
describe("FluxStore", () => {
  function getNewStore() {
    const mainModule = {
      ...
      mutations: {
        ...
        changeC(state: any, payload: any) {
          state.c = Object.assign({}, state.c, { a: payload });
        },
        changeCDirectly(state: any, payload: any) {
          state.c.a = payload;
        }
      },
      actions: {
        pushB(context: any, payload: any) {
      ...
      getters: {
        length(state: any, getters: any) {
          return getters.items.length;
        },
        items(state: any, getters: any) {
          return state.b;
        }
      }
    };
  ...
  it("Throw error if you try to listen with something other than a function", () => {
    const store = getNewStore();
    expect(() => store.on("set", "hello" as any)).toThrowError("You need to provide a valid function as listener.");
  });

  it("Throw error if you try to remove a listener that is something other than a function", () => {
    const store = getNewStore();
    expect(() => store.off("set", "hello" as any)).toThrowError("You need to provide a valid function as listener.");
  });
  ...
  it("Test all listeners", async () => {
    ...
    const methods = events.reduce((methods: any, event) => {
      methods[event] = {
        params: [],
        method: (...params: any) => methods[event].params.push(params)
      };
      return methods;
    }, {});
  ...
  it("Register nested modules", () => {
    ...
    expect((store as any).init.modules).toEqual({
  ...
  it("Call a root action and mutation from a nested module action", async () => {
    ...
    store.registerModule("my.module", {
      actions: {
        async change(store: any, payload) {
