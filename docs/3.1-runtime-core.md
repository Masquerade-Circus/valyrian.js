...
Use it when state changes happen outside delegated event handlers, when you call `event.preventDefault()` and want manual control of renders, or when you want to force an intermediate state at a specific time.
