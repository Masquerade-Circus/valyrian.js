...
- `server.ts` opens `ServerStorage.run(...)`, creates a per-request `request.new(...)`, resolves the route and renders HTML.
...
- In Node.js request context (`ServerStorage.run(...)`), the exported `request` singleton can also resolve the contextual scoped client automatically.
