...
Server-side rendering in Valyrian is not a separate product mode bolted onto a client framework. It is the server-side expression of the same runtime model used in the browser: render the same component tree, preserve the same state shape, and continue with the same patch and hydration rules after the HTML reaches the client.

In practice, SSR means running the Valyrian runtime through `valyrian.js/node` for each request, producing deterministic HTML, and then mounting the same app on the same container in the browser so hydration can continue from that existing DOM.
...
Use this pattern to ship one full browser/server runtime cycle (server render + browser hydration).
