...
- Common schema formats used in form validation include `email` and `url` (requires `formSchemaShield.addFormat` before creating the FormStore).
...
- `form.submitError` - Error thrown by `onSubmit` callback (null if none)
- `form.hasSubmitError` - Boolean, true if `submitError` is not null
- `submitError` is cleared to null before each submit attempt
