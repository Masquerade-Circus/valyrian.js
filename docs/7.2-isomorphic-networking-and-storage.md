...
Use the same request and storage APIs across browser and Node.js. In Node.js, create request-scoped clients and storage boundaries so concurrent renders cannot share user state while the application code keeps one request and storage model.
...
Absolute URL support is intentional. `request` is an HTTP client, not an internal router, so it may call external APIs, local services, or server-only endpoints. In Node.js, absolute URLs that match `urls.api` are rewritten to `urls.node`; other absolute URLs are kept unchanged by design.
