...
var import_network = require("valyrian.js/network");
var import_native_store = require("valyrian.js/native-store");
var import_utils = require("valyrian.js/utils");
function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}
function toErrorMessage(error) {
  if ((0, import_utils.isString)(error)) {
    return error;
  }
...
var OfflineQueue = class {
  ...
  constructor(options) {
    ...
    const storageType = options.storage === "session" ? import_native_store.StorageType.Session : import_native_store.StorageType.Local;
    this.#store = (0, import_native_store.createNativeStore)(options.id, {}, storageType, true);
    ...
    this.#offOnline = this.#network.on(import_network.NetworkEvent.ONLINE, () => {
      void this.sync();
    });
  }
  #emit(event, payload) {
