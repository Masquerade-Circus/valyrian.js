...
async function inline(file, options = {}) {
  if (typeof file === "string") {
    ...
    if (ext && /(js|cjs|jsx|mjs|ts|tsx)/.test(ext)) {
      ...
      let result = await new import_clean_css.default({
        sourceMap: true,
        level: {
          1: {
...
inline.uncss = async function(renderedHtml, css, options = {}) {
  ...
  let purgecss = new import_purgecss.PurgeCSS();
  ...
  let cleanCss = await new import_clean_css.default({
    sourceMap: false,
    level: {
      1: {
        roundingPrecision: "all=3"
...
function sw(file, options = {}) {
  let swfiletemplate = import_path.default.resolve(__dirname, "./node.sw.tpl");
  let swTpl = import_fs3.default.readFileSync(swfiletemplate, "utf8");
