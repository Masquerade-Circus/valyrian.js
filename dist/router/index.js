...
async function searchComponent(router, middlewares) {
  ...
  for (const middleware of middlewares) {
    ...
    if (
      response !== void 0 &&
      ((0, import_valyrian.isComponent)(response) || (0, import_valyrian.isVnodeComponent)(response))
    ) {
      return response;
    }
    if (response === false) {
...
var Router = class {
  ...
  add(path, ...middlewares) {
    let pathWithoutLastSlash = `${this.pathPrefix}${path}`.replace(/\/$/, "");
  ...
  async go(path, parentComponent, preventPushState = false) {
    ...
    if ((0, import_valyrian.isComponent)(parentComponent) || (0, import_valyrian.isVnodeComponent)(parentComponent)) {
      const childComponent = (0, import_valyrian.isVnodeComponent)(component)
        ? component
        : (0, import_valyrian.v)(component, {});
